# based on https://github.com/NixOS/nixpkgs/blob/master/pkgs/servers/dns/coredns/default.nix
# fixed plugin installation ("go get" in modBuildPhase. i hate golang) and added assertion

let
  defaultHash = "sha256-tp22jj6DNnYFQhtAFW2uLo10ty//dyNqIDH2egDgbOw=";
in
{ lib
, stdenv
, buildGoModule
...
, installShellFiles
, externalPlugins ? [ ]
, vendorHash ? defaultHash
}:

assert lib.assertMsg (builtins.length externalPlugins > 0 -> vendorHash != defaultHash) "Vendor hash must be changed when using external plugins";
